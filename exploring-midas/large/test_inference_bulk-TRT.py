#!/usr/bin/env python3
"""
Batch inference script for unified multitask models — TensorRT version.
Runs depth and segmentation on all images in an input folder,
...
# --- Configuration ---
DEPTH_TRT = "/home/albus/Documents/AureliusIndustries/miracle/Code/large/depth_teacher.trt"
SEG_TRT   = "/home/albus/Documents/AureliusIndustries/miracle/Code/large/seg_teacher.trt"
...
INTEREST_IDS = [
    UNIFIED_NAME_TO_ID[name]
    for name in SEGMENTATION_INTEREST_CLASSES
...
class TRTModel:
    """Minimal TensorRT inference wrapper compatible with the existing pipeline."""
    ...
    def __init__(self, engine_path: str):
        ...
        print(f"   TRT engine '{Path(engine_path).name}' — "
              f"input={self.input_name}, output={self.output_name}, out_shape={out_shape}")

    def __call__(self, x: torch.Tensor, return_backbone: bool = False):
        x = x.contiguous()
...
# --- Model Loading ---
def load_models():
    print(f"📦 Loading TRT Engines on {DEVICE}...")
...
# --- Per-Image Inference ---
def run_inference(frame: np.ndarray, depth_model: TRTModel, seg_model: TRTModel) -> tuple:
    ...
    for i, bbox in enumerate(bounding_boxes):
        ...
        cv2.putText(detection_vis, label, (bx1, by1 - 5),
...
def process_folder(input_dir: Path, output_dir: Path,
                   depth_model: TRTModel, seg_model: TRTModel) -> None:
    ...
    for idx, img_path in enumerate(image_paths, start=1):
        ...
        for key, img in frames_dict.items():
            cv2.imwrite(str(img_out_dir / f"{key}.jpg"), img)

        # Save per-image metadata
        metadata["image_name"] = img_path.name
        with open(img_out_dir / "metadata.json", "w") as f:
    ...
    if avg_latency is not None:
        print(f"   Inference time : {avg_latency:.1f} ms/image (GPU, TRT)")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch TRT inference: depth and segmentation on a folder of images."
    )
    parser.add_argument("input_dir",    type=Path,
                        help="Path to the folder containing input images.")
    parser.add_argument("--output_dir", type=Path, default=None,
                        help="Root output folder. Defaults to '<input_dir>_trt_results'.")
...
def main() -> None:
    ...
    output_dir: Path = (
        ...
        if args.output_dir
        else input_dir.parent / f"{input_dir.name}_trt_results"
    )
